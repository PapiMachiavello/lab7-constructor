...
# ============================================================
# Lab 7 — Character Generator (Constructors) Test Runner
# ============================================================
...
BUILD_DIR="$ROOT_DIR/build/character"
...
SRC="$ROOT_DIR/assignments/character/Character.java"
TEST="$ROOT_DIR/tests/CharacterTest.java"
...
compile_all() {
    mkdir -p "$BUILD_DIR"
    echo -e "${DIM}[1/3] Character.java компайл хийж байна...${RESET}"
    javac -d "$BUILD_DIR" "$SRC" 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}  ✗ Character компайл алдаатай!${RESET}"
        exit 1
    fi
    echo -e "${GREEN}  ✓ Character компайл амжилттай${RESET}"

    echo -e "${DIM}[2/3] CharacterTest.java компайл хийж байна...${RESET}"
...
main() {
    echo -e "${BOLD}${MAGENTA}"
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║  🐉 Lab 7 — Character Generator Grader       ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${RESET}"
